...
// ---------------------------------------------------------------------------
// Site mode + book preset (one series, no hub)
// ---------------------------------------------------------------------------

/**
 * Which preset the content folder uses:
 *  - hub: content/home/home.md + content/projects/<slug>/ (the catalog)
 *  - book: content/home.md + content/books/ (one series, no hub)
 * Detected by which files exist, so content can live anywhere unchanged.
 */
export type SiteMode = 'hub' | 'book';

export async function getSiteMode(): Promise<SiteMode> {
	const homes = await getCollection('seriesHome');
	return homes.length > 0 ? 'book' : 'hub';
}

export type SeriesHome = CollectionEntry<'seriesHome'>;
export type DirectBook = CollectionEntry<'directBooks'>;
export type DirectChapter = CollectionEntry<'directChapters'>;

export async function getSeriesHome(): Promise<SeriesHome | undefined> {
	return (await getCollection('seriesHome'))[0];
}

/** The books of the (single) series, in reading order. Entry id = book folder. */
export async function getDirectBooks(): Promise<DirectBook[]> {
	return (await getCollection('directBooks')).sort(
		(a, b) => a.data.order - b.data.order || a.id.localeCompare(b.id),
	);
}

export async function getDirectBook(key: string): Promise<DirectBook | undefined> {
	return (await getCollection('directBooks')).find((b) => b.id === key);
}

/** The chapters inside one book (entry id = the book folder). */
export async function getDirectChaptersInBook(bookKey: string): Promise<DirectChapter[]> {
	return (await getCollection('directChapters'))
		.filter((c) => c.id.startsWith(`${bookKey}/`))
		.sort((a, b) => a.data.order - b.data.order || a.id.localeCompare(b.id));
}

/** How many chapters a direct (book-preset) book has. */
export async function countDirectChapters(bookKey: string): Promise<number> {
	return (await getDirectChaptersInBook(bookKey)).length;
}

/** Root-relative URLs for the book preset (the site IS the series: `/` is its home). */
export const directBookHref = (bookKey: string): string => siteHref(`/books/${bookKey}/`);
export const directChapterHref = (bookKey: string, name: string): string =>
	siteHref(`/books/${bookKey}/chapters/${name}/`);
